...
describe('ModalConnector', () => {
  ...
  beforeEach(() => {
    ;(EthereumProvider.init as unknown as jest.Mock).mockResolvedValue(
      mockWalletConnectProvider
    )
    ;(providers.Web3Provider as unknown as jest.Mock).mockImplementation(
      () => mockProvider
    )
  ...
  it('should activate the connection to the wallet', async () => {
    ...
    expect(modalConnector.subscribeToEvents).toHaveBeenCalledWith(
      mockWalletConnectProvider
    )
  ...
  it('by default, if requestAccount is not passed, activate method should call requestAccount.', async () => {
    const connector = new ModalConnector({
      ...options,
    ...
    expect(mockProvider.getNetwork).toHaveBeenCalledTimes(1)
    expect(connector.subscribeToEvents).toHaveBeenCalledWith(
      mockWalletConnectProvider
    )
  ...
  it('requestAccount should call to enable walletconnect provider', async () => {
    ...
    jest
      .spyOn(modalConnector, 'walletConnectProvider', 'get')
      .mockReturnValueOnce(mockWalletConnectProvider as unknown as ETHProvider)
      .mockReturnValueOnce(mockWalletConnectProvider as unknown as ETHProvider)

    jest
      .spyOn(modalConnector, 'provider', 'get')
  ...
  it('requestAccount should fail if no provider', async () => {
    jest.spyOn(modalConnector, 'provider', 'get').mockReturnValueOnce(undefined)
  ...
  it('deactivate should throw an error if there is no provider', async () => {
    ...
    await expect(modalConnector.deactivate()).rejects.toThrow(
      errors.NO_ETHEREUM
    )
  })
